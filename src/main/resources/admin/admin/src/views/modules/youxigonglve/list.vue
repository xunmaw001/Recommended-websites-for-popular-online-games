		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"minHeight":"100vh","padding":"10px 20px 20px","boxShadow":"0 0px 0px rgba(0,0,0,.1)","margin":"0px 0 0","flexWrap":"wrap","textAlign":"center","background":"#fff","display":"block","width":"16%","float":"left","order":"1"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"block"}' >
					...
					<div :style='{"margin":"0 0px 10px","display":"inline-block"}' class="select" label="游戏类型" prop="youxileixing">
						<label :style='{"margin":"0 10px 0 0","color":"#666","textAlign":"left","display":"inline-block","width":"100%","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">游戏类型</label>
						<el-select  @change="youxileixingChange" clearable v-model="searchForm.youxileixing" placeholder="请选择游戏类型">
							<el-option v-for="(item,index) in youxileixingOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row :style='{"padding":"10px 20px","margin":"0px 0 0px","top":"10px","left":"calc(16% + 10px)","background":"#f6f6f6","display":"flex","width":"auto","position":"absolute"}'>
					<el-button :style='{"border":"1px solid #a2b1e8","cursor":"pointer","padding":"0 14px","boxShadow":"0 2px 6px rgba(0,0,0,.1)","margin":"0 8px 10px 0","outline":"none","color":"#a2b1e8","borderRadius":"0px","background":"rgba(243,234,250,.2)","width":"auto","fontSize":"14px","height":"36px"}' v-if="isAuth('youxigonglve','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"1px solid #d67081","cursor":"pointer","padding":"0 14px","boxShadow":"0 2px 6px rgba(0,0,0,.1)","margin":"0 8px 10px 0","outline":"none","color":"#df2543","borderRadius":"0px","background":"rgba(223,37,67,.1)","width":"auto","fontSize":"14px","height":"36px"}' v-if="isAuth('youxigonglve','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					label="攻略关卡">
						<template slot-scope="scope">
							{{scope.row.gonglveguanka}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="gonglveshipin" label="攻略视频">
						<template slot-scope="scope">
							<el-button v-if="scope.row.gonglveshipin" type="text" size="small" @click="download($base.url+scope.row.gonglveshipin)">预览</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"0px solid #a347ea","cursor":"pointer","padding":"0 16px","margin":"4px 6px 4px 0","outline":"none","color":"#ea8501","borderRadius":"0px","background":"rgba(234,133,1,.1)","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('youxigonglve','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"border":"0px solid #1fbfae","cursor":"pointer","padding":"0 16px","margin":"4px 6px 4px 0","outline":"none","color":"#4d9be3","borderRadius":"0px","background":"rgba(77,155,227,.1)","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('youxigonglve','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>





							<el-button :style='{"border":"0px solid #dc333b","cursor":"pointer","padding":"0 16px","margin":"4px 6px 4px 0","outline":"none","color":"#dc333b","borderRadius":"0px","background":"rgba(220,51,59,.1)","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('youxigonglve','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
