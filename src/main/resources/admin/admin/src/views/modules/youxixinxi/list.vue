<template>
	<div class="main-content" :style='{"padding":"0px 0px","margin":"0px auto 0 auto","borderColor":"#eee","overflow":"hidden","background":"#f6f6f6","borderWidth":"1px 0 0","width":"94%","position":"relative","borderStyle":"solid"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"minHeight":"100vh","padding":"10px 20px 20px","boxShadow":"0 0px 0px rgba(0,0,0,.1)","margin":"0px 0 0","flexWrap":"wrap","textAlign":"center","background":"#fff","display":"block","width":"16%","float":"left","order":"1"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"padding":"10px 20px","margin":"0px 0 0px","top":"10px","left":"calc(16% + 10px)","background":"#f6f6f6","display":"flex","width":"auto","position":"absolute"}'>
					<el-button :style='{"border":"1px solid #a2b1e8","cursor":"pointer","padding":"0 14px","boxShadow":"0 2px 6px rgba(0,0,0,.1)","margin":"0 8px 10px 0","outline":"none","color":"#a2b1e8","borderRadius":"0px","background":"rgba(243,234,250,.2)","width":"auto","fontSize":"14px","height":"36px"}' v-if="isAuth('youxixinxi','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"1px solid #d67081","cursor":"pointer","padding":"0 14px","boxShadow":"0 2px 6px rgba(0,0,0,.1)","margin":"0 8px 10px 0","outline":"none","color":"#df2543","borderRadius":"0px","background":"rgba(223,37,67,.1)","width":"auto","fontSize":"14px","height":"36px"}' v-if="isAuth('youxixinxi','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="shipinjieshao" label="视频介绍">
						<template slot-scope="scope">
							<el-button v-if="scope.row.shipinjieshao" type="text" size="small" @click="download($base.url+scope.row.shipinjieshao)">预览</el-button>
                            <span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"0px solid #a347ea","cursor":"pointer","padding":"0 16px","margin":"4px 6px 4px 0","outline":"none","color":"#ea8501","borderRadius":"0px","background":"rgba(234,133,1,.1)","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('youxixinxi','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"border":"0px solid #a347ea","cursor":"pointer","padding":"0 16px","margin":"4px 6px 4px 0","outline":"none","color":"#ea8501","borderRadius":"0px","background":"rgba(234,133,1,.1)","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('youxixinxi','购买')" type="success" size="mini" @click="youxigoumaiCrossAddOrUpdateHandler(scope.row,'cross','','','')">购买</el-button>
							<el-button :style='{"border":"0px solid #1fbfae","cursor":"pointer","padding":"0 16px","margin":"4px 6px 4px 0","outline":"none","color":"#4d9be3","borderRadius":"0px","background":"rgba(77,155,227,.1)","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('youxixinxi','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>


							<el-button :style='{"border":"0px solid #a347ea","cursor":"pointer","padding":"0 16px","margin":"4px 6px 4px 0","outline":"none","color":"#ea8501","borderRadius":"0px","background":"rgba(234,133,1,.1)","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('youxixinxi','查看评论')" type="primary" size="mini" @click="disscussListHandler(scope.row.id)">查看评论</el-button>



							<el-button :style='{"border":"0px solid #dc333b","cursor":"pointer","padding":"0 16px","margin":"4px 6px 4px 0","outline":"none","color":"#dc333b","borderRadius":"0px","background":"rgba(220,51,59,.1)","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('youxixinxi','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
		...
		<youxigoumai-cross-add-or-update v-if="youxigoumaiCrossAddOrUpdateFlag" :parent="this" ref="youxigoumaiCrossaddOrUpdate"></youxigoumai-cross-add-or-update>
...
import youxigoumaiCrossAddOrUpdate from "../youxigoumai/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      youxigoumaiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    youxigoumaiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.youxigoumaiCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.youxigoumaiCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.youxigoumaiCrossaddOrUpdate.init(row.id,type);
      });
    },
