...
<div :style='{"width":"1200px","padding":"20px","margin":"0px auto","position":"relative","background":"none"}'>
    ...
    >
          ...
          <el-form-item :style='{"border":"3px solid #c6ecc6","padding":"10px","margin":"0 0 10px","background":"#fff"}' label="攻略关卡" prop="gonglveguanka">
            <el-input v-model="ruleForm.gonglveguanka" 
                placeholder="攻略关卡" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"border":"3px solid #c6ecc6","padding":"10px","margin":"0 0 10px","background":"#fff"}' label="攻略视频" prop="gonglveshipin">
            <file-upload
            tip="点击上传攻略视频"
            action="file/upload"
            :limit="1"
          ...
          <el-form-item :style='{"border":"3px solid #c6ecc6","padding":"10px","margin":"0 0 10px","background":"#fff"}' label="游戏攻略" prop="youxigonglve">
            <editor 
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('youxigonglve/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('youxigonglve/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('youxigonglve/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
      ...
      gonglveshipinUploadChange(fileUrls) {
          this.ruleForm.gonglveshipin = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
