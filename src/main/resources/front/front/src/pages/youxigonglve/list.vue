...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"30px 10px 0","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>游戏名称</div>
        <el-input v-model="formSearch.youximingcheng" placeholder="游戏名称" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
		<div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>游戏类型</div>
        <el-select v-model="formSearch.youxileixing" placeholder="请选择游戏类型" :clearable="true">
          <el-option v-for="(item, index) in youxileixingOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('youxigonglve','新增')" :style='{"cursor":"pointer","padding":"0px 10px","margin":"0 4px 0 0","borderColor":"#fda100","color":"#fda100","outline":"none","borderRadius":"4px","background":"#fcf2e1","borderWidth":"1px 4px","width":"auto","fontSize":"14px","lineHeight":"42px","borderStyle":"solid","height":"42px"}' type="primary" @click="add('/index/youxigonglveAdd')"><i v-if="true" :style='{"color":"#fda100","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"margin":"20px 0 10px","background":"none"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0 10px","margin":"30px 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"padding":"10px","boxShadow":"0px 2px 6px #e6e6e6","margin":"0 0 20px","borderRadius":"8px","flexWrap":"wrap","background":"#fff","display":"flex","width":"23.5%","position":"relative","justifyContent":"space-between","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"cursor":"pointer","width":"100%","margin":"0 0 8px","objectFit":"cover","display":"block","height":"275px"}' v-if="item.youxifengmian && item.youxifengmian.substr(0,4)=='http'" :src="item.youxifengmian" class="image" />
				<img :style='{"cursor":"pointer","width":"100%","margin":"0 0 8px","objectFit":"cover","display":"block","height":"275px"}' v-else :src="baseUrl + (item.youxifengmian?item.youxifengmian.split(',')[0]:'')" class="image" />
				...
				<div :style='{"width":"100%","padding":"4px 10px","lineHeight":"24px","fontSize":"16px","color":"#249940","textAlign":"center"}' class="name ">游戏名称:{{item.youximingcheng}}</div>
			</div>
		</div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/youxileixing/youxileixing').then(res => {
        if (res.data.code == 0) {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.youxileixing != '') searchWhere.youxileixing = this.formSearch.youxileixing;
        this.$http.get('youxigonglve/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/youxigonglveDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
